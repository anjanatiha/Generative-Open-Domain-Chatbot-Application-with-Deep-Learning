...
# writes input output file
def dialouge_seperator_cornell_movie_o(raw_dialogue_file_path, dialogue_file1, dialogue_file2, symbol_seq, max_dialouge_count):
...
def dialouge_seperator_cornell_movie(raw_dialogue_file_path, dialogue_file1, dialogue_file2, symbol_seq, max_dialouge_count):
	...
	for line in reversed(list(open(raw_dialogue_file_path))):
		line_original = line
		temp = line.split("+++$+++")
		if temp[2] != current_movie_id:
			current_movie_id = temp[2]
			p2 = ""
		if current_character == temp[3]:
			p2 = ""
		else:
			current_character = temp[3]

		#print(current_movie_id, "------------------->>", temp)
