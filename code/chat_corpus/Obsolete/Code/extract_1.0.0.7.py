...
def train_test_split(input_file1, input_file2, max_count, percent, train_file1, train_file2, test_file1, test_file2, dev_file1, dev_file2):
    content1 = read_file_line_by_line(input_file1)
    content2 = read_file_line_by_line(input_file2)
    if len(content1) != content2:
    	print("Two files dont have same length")

    if max_count == -1:
    	max_count = len(content1)

    train1 = open(train_file1,"w")
    train2 = open(train_file2,"w")
    test1 = open(test_file1,"w")
    test2 = open(test_file2,"w")
    dev1 = open(dev_file1,"w")
    dev2 = open(dev_file2,"w")

    train_end = (max_count*(percent/100)) - 1
    test_end =  train_end + ((max_count - train_end)/2)
    ...
    for line in content1:
        ...
        if dialouge_count >= max_count and max_count != -1:
            break
...
output_file1 = "train_test.en"
output_file2 = "train_test.vi"
...
def file_dir_name_fix(parent_dir, extracted_dir, pre_processed_dir, movie_dir, output_file1, output_file2, vocab_file1, vocab_file2):
	...
	out_dir_path = parent_dir+ "/" + pre_processed_dir + "/" + movie_dir
	input_file_path = parent_dir+ "/" + extracted_dir + "/" + movie_dir + ".txt"
	output_file1 = parent_dir+ "/" + pre_processed_dir + "/" + movie_dir + "/" + output_file1
	output_file2 = parent_dir+ "/" + pre_processed_dir + "/" + movie_dir + "/" + output_file2
	vocab_file1 = parent_dir+ "/" + input_data_dir + "/" + movie_dir + "/" + vocab_file1
	vocab_file2 = parent_dir+ "/" + input_data_dir + "/" + movie_dir + "/" + vocab_file2
...
_out_dir_path, _input_file_path, _output_file1, _output_file2, _vocab_file1, _vocab_file2 = file_dir_name_fix(parent_dir, extracted_dir, pre_processed_dir, movie_dir, output_file1, output_file2, vocab_file1, vocab_file2)
pre_process_dialouge(_out_dir_path, _input_file_path, movie_dir, _output_file1, _output_file2, _vocab_file1, _vocab_file2, dialouge_count, type_d, symbol_seq, compare_count)
